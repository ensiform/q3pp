...
sfxHandle_t	CG_CustomSound( int clientNum, const char *soundName ) {
	...
	if ( soundName[0] != '*' ) {
		return trap->si->RegisterSound( soundName, false );
	}
...
==========================
*/
static bool	CG_FindClientModelFile( char *filename, int length, clientInfo_t *ci, const char *teamName, const char *modelName, const char *skinName, const char *base, const char *ext ) {
...
==========================
*/
static bool	CG_FindClientHeadFile( char *filename, int length, clientInfo_t *ci, const char *teamName, const char *headModelName, const char *headSkinName, const char *base, const char *ext ) {
...
static bool	CG_RegisterClientSkin( clientInfo_t *ci, const char *teamName, const char *modelName, const char *skinName, const char *headModelName, const char *headSkinName ) {
	...
	ci->legsSkin = trap->re->RegisterSkin( filename );
	if (!ci->legsSkin) {
	...
	ci->torsoSkin = trap->re->RegisterSkin( filename );
	if (!ci->torsoSkin) {
	...
	*/
	...
	if ( CG_FindClientHeadFile( filename, sizeof(filename), ci, teamName, headModelName, headSkinName, "head", "skin" ) ) {
		ci->headSkin = trap->re->RegisterSkin( filename );
	}
	if (!ci->headSkin) {
...
static bool CG_RegisterClientModelname( clientInfo_t *ci, const char *modelName, const char *skinName, const char *headModelName, const char *headSkinName, const char *teamName ) {
	...
	ci->legsModel = trap->re->RegisterModel( filename );
	if ( !ci->legsModel ) {
	...
	ci->torsoModel = trap->re->RegisterModel( filename );
	if ( !ci->torsoModel ) {
	...
	ci->headModel = trap->re->RegisterModel( filename );
	...
	if ( CG_FindClientHeadFile( filename, sizeof(filename), ci, teamName, headName, headSkinName, "icon", "skin" ) ) {
		ci->modelIcon = trap->re->RegisterShaderNoMip( filename );
	}
...
static void CG_LoadClientInfo( int clientNum, clientInfo_t *ci ) {
	...
	if ( ci->torsoModel ) {
		...
		if ( trap->re->LerpTag( &tag, ci->torsoModel, 0, 0, 1, "tag_flag" ) ) {
			ci->newAnims = true;
		}
	}
	...
	for ( i = 0 ; i < MAX_CUSTOM_SOUNDS ; i++ ) {
		...
		if (modelloaded) {
			ci->sounds[i] = trap->si->RegisterSound( va("sound/player/%s/%s", dir, s + 1), false );
		}
		if ( !ci->sounds[i] ) {
			ci->sounds[i] = trap->si->RegisterSound( va("sound/player/%s/%s", fallback, s + 1), false );
		}
	}
...
void CG_NewClientInfo( int clientNum ) {
	...
	if ( cg_forceModel.integer ) {
		...
		if( cgs.gametype >= GT_TEAM ) {
			...
			cvarSystem->VariableStringBuffer( "model", modelStr, sizeof( modelStr ) );
	...
	if ( cg_forceModel.integer ) {
		...
		if( cgs.gametype >= GT_TEAM ) {
			...
			cvarSystem->VariableStringBuffer( "headmodel", modelStr, sizeof( modelStr ) );
			if ( ( skin = strchr( modelStr, '/' ) ) == NULL) {
				skin = "default";
			} else {
				*skin++ = 0;
	...
	if ( !CG_ScanForExistingClientInfo( &newInfo ) ) {
		...
		forceDefer = trap->MemoryRemaining() < 4000000;
...
void CG_LoadDeferredPlayers( void ) {
	...
	for ( i = 0, ci = cgs.clientinfo ; i < cgs.maxclients ; i++, ci++ ) {
		if ( ci->infoValid && ci->deferred ) {
			...
			if ( trap->MemoryRemaining() < 4000000 ) {
				CG_Printf( "Memory is low. Using deferred model.\n" );
				ci->deferred = false;
				continue;
			}
...
static void CG_DustTrail( centity_t *cent ) {
	...
	CG_Trace( &tr, cent->currentState.pos.trBase, NULL, NULL, end, cent->currentState.number, MASK_PLAYERSOLID, false );
...
static void CG_PlayerFlag( centity_t *cent, qhandle_t hSkin, refEntity_t *torso ) {
	...
	trap->re->AddRefEntityToScene( &pole );
	...
	trap->re->AddRefEntityToScene( &flag );
}
...
static void CG_PlayerPowerups( centity_t *cent, refEntity_t *torso ) {
	...
	if ( powerups & ( 1 << PW_QUAD ) ) {
		trap->re->AddLightToScene( cent->lerpOrigin, 200 + (rand()&31), 0.2f, 0.2f, 1 );
	}

	// flight plays a looped sound
	if ( powerups & ( 1 << PW_FLIGHT ) ) {
		trap->si->AddLoopingSound( cent->currentState.number, cent->lerpOrigin, vec3_origin, cgs.media.flightSound );
	}
	...
	if ( powerups & ( 1 << PW_REDFLAG ) ) {
		...
		trap->re->AddLightToScene( cent->lerpOrigin, 200 + (rand()&31), 1.0, 0.2f, 0.2f );
	}
	...
	if ( powerups & ( 1 << PW_BLUEFLAG ) ) {
		...
		trap->re->AddLightToScene( cent->lerpOrigin, 200 + (rand()&31), 0.2f, 0.2f, 1.0 );
	}
	...
	if ( powerups & ( 1 << PW_NEUTRALFLAG ) ) {
		...
		trap->re->AddLightToScene( cent->lerpOrigin, 200 + (rand()&31), 1.0, 1.0, 1.0 );
	}
...
static bool CG_PlayerShadow( centity_t *cent, float *shadowPlane ) {
	...
	*shadowPlane = 0;
	...
	trap->CM_BoxTrace( &trace, cent->lerpOrigin, end, mins, maxs, 0, MASK_PLAYERSOLID, false );
...
static void CG_PlayerSplash( centity_t *cent ) {
	...
	trap->CM_BoxTrace( &trace, start, end, NULL, NULL, 0, ( CONTENTS_WATER | CONTENTS_SLIME | CONTENTS_LAVA ), false );
	...
	trap->re->AddPolyToScene( cgs.media.wakeMarkShader, 4, verts, 1 );
}
...
int CG_LightVerts( vec3_t normal, int numVerts, polyVert_t *verts )
{
	...
	trap->re->LightForPoint( verts[0].xyz, ambientLight, directedLight, lightDir );
...
void CG_Player( centity_t *cent ) {
	...
	if ( cent->currentState.eFlags & EF_KAMIKAZE ) {
		...
		if ( cent->currentState.eFlags & EF_DEAD ) {
			...
			trap->re->AddRefEntityToScene( &skull );
	...
	if ( cent->currentState.powerups & ( 1 << PW_GUARD ) ) {
		...
		trap->re->AddRefEntityToScene( &powerup );
	}
