...
static void CG_SetSelectedPlayerName( void ) {
  if (cg_currentSelectedPlayer.integer >= 0 && cg_currentSelectedPlayer.integer < numSortedTeamPlayers) {
		clientInfo_t *ci = cgs.clientinfo + sortedTeamPlayers[cg_currentSelectedPlayer.integer];
	  if (ci) {
			cvarSystem->Set("cg_selectedPlayerName", ci->name);
			cvarSystem->Set("cg_selectedPlayer", va("%d", sortedTeamPlayers[cg_currentSelectedPlayer.integer]));
			cgs.currentOrder = ci->teamTask;
	  }
	} else {
		cvarSystem->Set("cg_selectedPlayerName", "Everyone");
	}
}
...
static void CG_DrawSelectedPlayerName( rectDef_t *rect, float scale, vec4_t color, bool voice, int textStyle) {
	clientInfo_t *ci;
    ...
    y = rect->y;
		...
		for (j = 0; j < PW_NUM_POWERUPS; j++) {
			if (ci->powerups & (1 << j)) {
				...
				if (item) {
				  CG_DrawPic( x, y, rect->w, rect->h, trap->re->RegisterShader( item->icon ) );
					x += 3;
					y += 3;
          return;
				}
...
static void CG_HarvesterSkulls(rectDef_t *rect, float scale, vec4_t color, bool force2D, int textStyle ) {
	char num[16];
	vec3_t origin, angles;
	qhandle_t handle;
...
static void CG_DrawAreaPowerUp(rectDef_t *rect, int align, float special, float scale, vec4_t color) {
	...
	for ( i = 0 ; i < active ; i++ ) {
		...
		if (item) {
			...
			CG_DrawPic( r2.x, r2.y, r2.w * .75, r2.h, trap->re->RegisterShader( item->icon ) );
		...
		if ( ci->infoValid && ci->team == cg.snap->ps.persistant[PERS_TEAM]) {
			...
			for (j = 0; j <= PW_NUM_POWERUPS; j++) {
				if (ci->powerups & (1 << j)) {
					...
					if (item) {
						CG_DrawPic( xx, y, PIC_WIDTH, PIC_WIDTH, trap->re->RegisterShader( item->icon ) );
						xx += PIC_WIDTH;
					}
				}
			}
...
void CG_ShowResponseHead(void) {
  Menus_OpenByName("voiceMenu");
	cvarSystem->Set("cl_conXOffset", "72");
