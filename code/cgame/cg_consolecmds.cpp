...
void CG_TargetCommand_f( void ) {
	...
	trap->SendClientCommand( va( "gc %i %i", targetNum, atoi( test ) ), false );
}
...
static void CG_SizeUp_f (void) {
	cvarSystem->Set("cg_viewsize", va("%i",(int)(cg_viewsize.integer+10)));
}
...
static void CG_ScoresDown_f( void ) {
	...
	if ( cg.scoresRequestTime + 2000 < cg.time ) {
		...
		trap->SendClientCommand( "score", false );
...
static void CG_spWin_f( void) {
	cvarSystem->Set("cg_cameraOrbit", "2");
	cvarSystem->Set("cg_cameraOrbitDelay", "35");
	cvarSystem->Set("cg_thirdPerson", "1");
	cvarSystem->Set("cg_thirdPersonAngle", "0");
	cvarSystem->Set("cg_thirdPersonRange", "100");
	CG_AddBufferedSound(cgs.media.winnerSound);
	//trap->si->StartLocalSound(cgs.media.winnerSound, CHAN_ANNOUNCER);
...
static void CG_spLose_f( void) {
	...
	//trap->si->StartLocalSound(cgs.media.loserSound, CHAN_ANNOUNCER);
...
static void CG_StartOrbit_f( void ) {
	...
	cvarSystem->VariableStringBuffer( "developer", var, sizeof( var ) );
	if ( !atoi(var) ) {
		return;
	}
	if (cg_cameraOrbit.value != 0) {
		cvarSystem->Set ("cg_cameraOrbit", "0");
		cvarSystem->Set("cg_thirdPerson", "0");
	} else {
		cvarSystem->Set("cg_cameraOrbit", "5");
		cvarSystem->Set("cg_thirdPerson", "1");
		cvarSystem->Set("cg_thirdPersonAngle", "0");
		cvarSystem->Set("cg_thirdPersonRange", "100");
	}
...
static void CG_Camera_f( void ) {
	...
	if (trap->loadCamera(name)) {
		cg.cameraMode = true;
		trap->startCamera(cg.time);
	} else {
...
void CG_InitConsoleCommands( void ) {
	...
	//
	trap->AddCommand ("kill");
	trap->AddCommand ("say");
	trap->AddCommand ("say_team");
	trap->AddCommand ("tell");
#ifdef MISSIONPACK
	trap->AddCommand ("vsay");
	trap->AddCommand ("vsay_team");
	trap->AddCommand ("vtell");
	trap->AddCommand ("vtaunt");
	trap->AddCommand ("vosay");
	trap->AddCommand ("vosay_team");
	trap->AddCommand ("votell");
#endif
	trap->AddCommand ("give");
	trap->AddCommand ("god");
	trap->AddCommand ("notarget");
	trap->AddCommand ("noclip");
	trap->AddCommand ("where");
	trap->AddCommand ("team");
	trap->AddCommand ("follow");
	trap->AddCommand ("follownext");
	trap->AddCommand ("followprev");
	trap->AddCommand ("levelshot");
	trap->AddCommand ("addbot");
	trap->AddCommand ("setviewpos");
	trap->AddCommand ("callvote");
	trap->AddCommand ("vote");
	trap->AddCommand ("callteamvote");
	trap->AddCommand ("teamvote");
	trap->AddCommand ("stats");
	trap->AddCommand ("teamtask");
	trap->AddCommand ("loaddefered");	// spelled wrong, but not changing for demo
}
