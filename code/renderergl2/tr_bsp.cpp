...
static void R_CreateWorldVBOs(void)
{
	...
	// presort surfaces
	surfacesSorted = (msurface_t **)ri->Malloc(numSortedSurfaces * sizeof(*surfacesSorted));
...
void R_MergeLeafSurfaces(void)
{
	...
	for (i = 0; i < numWorldSurfaces; i++)
	{
		...
		// allocate indexes
		iboIndexes = outIboIndexes = (glIndex_t *)ri->Malloc( numIndexes * sizeof(*outIboIndexes) );
