...
void BotAI_Trace(bsp_trace_t *bsptrace, vec3_t start, vec3_t mins, vec3_t maxs, vec3_t end, int passent, int contentmask) {
	...
	trap->Trace(&trace, start, mins, maxs, end, passent, contentmask, false);
...
void QDECL BotAI_BotInitialChat( bot_state_t *bs, char *type, ... ) {
	...
	botlib->ai.BotInitialChat( bs->cs, type, mcontext, vars[0], vars[1], vars[2], vars[3], vars[4], vars[5], vars[6], vars[7] );
}
...
void BotTestAAS(vec3_t origin) {
	...
	cvarSystem->Update(&bot_testsolid);
	cvarSystem->Update(&bot_testclusters);
	if (bot_testsolid.integer) {
		if (!botlib->aas.AAS_Initialized()) return;
		...
		if (!areanum)
			...
			botlib->aas.AAS_AreaInfo(areanum, &info);
...
void BotReportStatus(bot_state_t *bs) {
	...
	switch(bs->ltgtype) {
		...
		{
			botlib->ai.BotGoalName(bs->teamgoal.number, goalname, sizeof(goalname));
...
void BotSetInfoConfigString(bot_state_t *bs) {
	...
	switch(bs->ltgtype) {
		...
		default:
		{
			botlib->ai.BotGetTopGoal(bs->gs, &goal);
			botlib->ai.BotGoalName(goal.number, goalname, sizeof(goalname));
  	...
  	cs = va("l\\%s\\c\\%s\\a\\%s",
				...
				carrying,
				action);
  	trap->SetConfigstring (CS_BOTINFO + bs->client, cs);
}
...
void BotInterbreedBots(void) {
	...
	if (botlib->ai.GeneticParentsAndChildSelection(MAX_CLIENTS, ranks, &parent1, &parent2, &child)) {
		botlib->ai.BotInterbreedGoalFuzzyLogic(botstates[parent1]->gs, botstates[parent2]->gs, botstates[child]->gs);
		botlib->ai.BotMutateGoalFuzzyLogic(botstates[child]->gs, 1);
	}
...
void BotWriteInterbreeded(char *filename) {
	...
	if (bestbot >= 0) {
		...
		botlib->ai.BotSaveGoalFuzzyLogic(botstates[bestbot]->gs, filename);
	}
}
...
void BotInterbreeding(void) {
	...
	cvarSystem->Update(&bot_interbreedchar);
	if (!strlen(bot_interbreedchar.string)) return;
	//make sure we are in tournament mode
	if (gametype != GT_TOURNAMENT) {
		cvarSystem->Set("g_gametype", va("%d", GT_TOURNAMENT));
		ExitLevel();
		return;
	}
...
void BotEntityInfo(int entnum, aas_entityinfo_t *info) {
	botlib->aas.AAS_EntityInfo(entnum, info);
}
...
void BotChangeViewAngles(bot_state_t *bs, float thinktime) {
	...
	//
	if (bs->enemy >= 0) {
		factor = botlib->ai.Characteristic_BFloat(bs->character, CHARACTERISTIC_VIEW_FACTOR, 0.01f, 1);
		maxchange = botlib->ai.Characteristic_BFloat(bs->character, CHARACTERISTIC_VIEW_MAXCHANGE, 1, 1800);
	}
	else {
		factor = 0.05f;
		maxchange = 360;
...
void BotUpdateInput(bot_state_t *bs, int time, int elapsed_time) {
	...
	botlib->ea.EA_GetInput(bs->client, (float) time / 1000, &bi);
	//respawn hack
...
int BotAI(int client, float thinktime) {
	...
	botlib->ea.EA_ResetInput(client);
	//
	bs = botstates[client];
	if (!bs || !bs->inuse) {
	...
	while( trap->BotGetConsoleMessage(client, buf, sizeof(buf)) ) {
		...
		*args++ = '\0';
		...
		if (!Q_stricmp(buf, "cp "))
			...
			botlib->ai.BotQueueConsoleMessage(bs->cs, CMS_NORMAL, args);
		}
		else if (!Q_stricmp(buf, "chat")) {
			//remove first and last quote from the chat message
			memmove(args, args+1, strlen(args));
			args[strlen(args)-1] = '\0';
			botlib->ai.BotQueueConsoleMessage(bs->cs, CMS_CHAT, args);
		}
...
int BotAISetupClient(int client, struct bot_settings_s *settings, bool restart) {
	...
	if (!botlib->aas.AAS_Initialized()) {
		BotAI_Print(PRT_FATAL, "AAS not initialized\n");
		return false;
	}

	//load the bot character
	bs->character = botlib->ai.BotLoadCharacter(settings->characterfile, settings->skill);
	if (!bs->character) {
	...
	bs->gs = botlib->ai.BotAllocGoalState(client);
	//load the item weights
	botlib->ai.Characteristic_String(bs->character, CHARACTERISTIC_ITEMWEIGHTS, filename, MAX_PATH);
	errnum = botlib->ai.BotLoadItemWeights(bs->gs, filename);
	if (errnum != BLERR_NOERROR) {
		botlib->ai.BotFreeGoalState(bs->gs);
		return false;
	}
	//allocate a weapon state
	bs->ws = botlib->ai.BotAllocWeaponState();
	//load the weapon weights
	botlib->ai.Characteristic_String(bs->character, CHARACTERISTIC_WEAPONWEIGHTS, filename, MAX_PATH);
	errnum = botlib->ai.BotLoadWeaponWeights(bs->ws, filename);
	if (errnum != BLERR_NOERROR) {
		botlib->ai.BotFreeGoalState(bs->gs);
		botlib->ai.BotFreeWeaponState(bs->ws);
		return false;
	}
	//allocate a chat state
	bs->cs = botlib->ai.BotAllocChatState();
	//load the chat file
	botlib->ai.Characteristic_String(bs->character, CHARACTERISTIC_CHAT_FILE, filename, MAX_PATH);
	botlib->ai.Characteristic_String(bs->character, CHARACTERISTIC_CHAT_NAME, name, MAX_PATH);
	errnum = botlib->ai.BotLoadChatFile(bs->cs, filename, name);
	if (errnum != BLERR_NOERROR) {
	...
	botlib->ai.Characteristic_String(bs->character, CHARACTERISTIC_GENDER, gender, MAX_PATH);
	//set the chat gender
	if (*gender == 'f' || *gender == 'F') botlib->ai.BotSetChatGender(bs->cs, CHAT_GENDERFEMALE);
	else if (*gender == 'm' || *gender == 'M') botlib->ai.BotSetChatGender(bs->cs, CHAT_GENDERMALE);
	...
	bs->ms = botlib->ai.BotAllocMoveState();
	bs->walker = botlib->ai.Characteristic_BFloat(bs->character, CHARACTERISTIC_WALKER, 0, 1);
	numbots++;

	if (cvarSystem->VariableIntegerValue("bot_testichat")) {
		botlib->BotLibVarSet("bot_testichat", "1");
		BotChatTest(bs);
	}
	...
	if (bot_interbreed) {
		botlib->ai.BotMutateGoalFuzzyLogic(bs->gs, 1);
	}
...
int BotAIShutdownClient(int client, bool restart) {
	...
	if (BotChat_ExitGame(bs)) {
		botlib->ai.BotEnterChat(bs->cs, bs->client, CHAT_ALL);
	}
	...
	botlib->ai.BotFreeCharacter(bs->character);
	//
...
void BotResetState(bot_state_t *bs) {
	...
	if (bs->ms) botlib->ai.BotResetMoveState(bs->ms);
	if (bs->gs) botlib->ai.BotResetGoalState(bs->gs);
	if (bs->ws) botlib->ai.BotResetWeaponState(bs->ws);
	if (bs->gs) botlib->ai.BotResetAvoidGoals(bs->gs);
	if (bs->ms) botlib->ai.BotResetAvoidReach(bs->ms);
}
...
int BotAILoadMap( bool restart ) {
	...
	if (!restart) {
		cvarSystem->Register( &mapname, "mapname", "", CVAR_SERVERINFO | CVAR_ROM );
		botlib->BotLibLoadMap( mapname.string );
	}
...
int BotAIStartFrame(int time) {
	...
	cvarSystem->Update(&bot_rocketjump);
	cvarSystem->Update(&bot_grapple);
	cvarSystem->Update(&bot_fastchat);
	cvarSystem->Update(&bot_nochat);
	cvarSystem->Update(&bot_testrchat);
	cvarSystem->Update(&bot_thinktime);
	cvarSystem->Update(&bot_memorydump);
	cvarSystem->Update(&bot_saveroutingcache);
	cvarSystem->Update(&bot_pause);
	cvarSystem->Update(&bot_report);

	if (bot_report.integer) {
//		BotTeamplayReport();
//		cvarSystem->Set("bot_report", "0");
		BotUpdateInfoConfigStrings();
	}
	...
	if (bot_memorydump.integer) {
		botlib->BotLibVarSet("memorydump", "1");
		cvarSystem->Set("bot_memorydump", "0");
	}
	if (bot_saveroutingcache.integer) {
		botlib->BotLibVarSet("saveroutingcache", "1");
		cvarSystem->Set("bot_saveroutingcache", "0");
	}
	//check if bot interbreeding is activated
	BotInterbreeding();
	//cap the bot think time
	if (bot_thinktime.integer > 200) {
		cvarSystem->Set("bot_thinktime", "200");
	}
	...
	if ( botlib_residual >= thinktime ) {
		...
		botlib->BotLibStartFrame((float) time / 1000);

		if (!botlib->aas.AAS_Initialized()) return false;

		//update entities in the botlib
		for (i = 0; i < MAX_GENTITIES; i++) {
			ent = &g_entities[i];
			if (!ent->inuse) {
				botlib->BotLibUpdateEntity(i, NULL);
				continue;
			}
			if (!ent->r.linked) {
				botlib->BotLibUpdateEntity(i, NULL);
			...
			//
			botlib->BotLibUpdateEntity(i, &state);
		}

		BotAIRegularUpdate();
	}

	floattime = botlib->aas.AAS_Time();
...
int BotInitLibrary(void) {
	...
	cvarSystem->VariableStringBuffer("sv_maxclients", buf, sizeof(buf));
	if (!strlen(buf)) strcpy(buf, "8");
	botlib->BotLibVarSet("maxclients", buf);
	Com_sprintf(buf, sizeof(buf), "%d", MAX_GENTITIES);
	botlib->BotLibVarSet("maxentities", buf);
	//bsp checksum
	cvarSystem->VariableStringBuffer("sv_mapChecksum", buf, sizeof(buf));
	if (strlen(buf)) botlib->BotLibVarSet("sv_mapChecksum", buf);
	//maximum number of aas links
	cvarSystem->VariableStringBuffer("max_aaslinks", buf, sizeof(buf));
	if (strlen(buf)) botlib->BotLibVarSet("max_aaslinks", buf);
	//maximum number of items in a level
	cvarSystem->VariableStringBuffer("max_levelitems", buf, sizeof(buf));
	if (strlen(buf)) botlib->BotLibVarSet("max_levelitems", buf);
	//game type
	cvarSystem->VariableStringBuffer("g_gametype", buf, sizeof(buf));
	if (!strlen(buf)) strcpy(buf, "0");
	botlib->BotLibVarSet("g_gametype", buf);
	//bot developer mode and log file
	botlib->BotLibVarSet("bot_developer", bot_developer.string);
	cvarSystem->VariableStringBuffer("logfile", buf, sizeof(buf));
	botlib->BotLibVarSet("log", buf);
	//no chatting
	cvarSystem->VariableStringBuffer("bot_nochat", buf, sizeof(buf));
	if (strlen(buf)) botlib->BotLibVarSet("nochat", buf);
	//visualize jump pads
	cvarSystem->VariableStringBuffer("bot_visualizejumppads", buf, sizeof(buf));
	if (strlen(buf)) botlib->BotLibVarSet("bot_visualizejumppads", buf);
	//forced clustering calculations
	cvarSystem->VariableStringBuffer("bot_forceclustering", buf, sizeof(buf));
	if (strlen(buf)) botlib->BotLibVarSet("forceclustering", buf);
	//forced reachability calculations
	cvarSystem->VariableStringBuffer("bot_forcereachability", buf, sizeof(buf));
	if (strlen(buf)) botlib->BotLibVarSet("forcereachability", buf);
	//force writing of AAS to file
	cvarSystem->VariableStringBuffer("bot_forcewrite", buf, sizeof(buf));
	if (strlen(buf)) botlib->BotLibVarSet("forcewrite", buf);
	//no AAS optimization
	cvarSystem->VariableStringBuffer("bot_aasoptimize", buf, sizeof(buf));
	if (strlen(buf)) botlib->BotLibVarSet("aasoptimize", buf);
	//
	cvarSystem->VariableStringBuffer("bot_saveroutingcache", buf, sizeof(buf));
	if (strlen(buf)) botlib->BotLibVarSet("saveroutingcache", buf);
	//reload instead of cache bot character files
	cvarSystem->VariableStringBuffer("bot_reloadcharacters", buf, sizeof(buf));
	if (!strlen(buf)) strcpy(buf, "0");
	botlib->BotLibVarSet("bot_reloadcharacters", buf);
	//base directory
	cvarSystem->VariableStringBuffer("fs_basepath", buf, sizeof(buf));
	if (strlen(buf)) botlib->BotLibVarSet("basedir", buf);
	//game directory
	cvarSystem->VariableStringBuffer("fs_game", buf, sizeof(buf));
	if (strlen(buf)) botlib->BotLibVarSet("gamedir", buf);
	//home directory
	cvarSystem->VariableStringBuffer("fs_homepath", buf, sizeof(buf));
...
int BotAISetup( bool restart ) {
	...
	cvarSystem->Register(&bot_thinktime, "bot_thinktime", "100", CVAR_CHEAT);
	cvarSystem->Register(&bot_memorydump, "bot_memorydump", "0", CVAR_CHEAT);
	cvarSystem->Register(&bot_saveroutingcache, "bot_saveroutingcache", "0", CVAR_CHEAT);
	cvarSystem->Register(&bot_pause, "bot_pause", "0", CVAR_CHEAT);
	cvarSystem->Register(&bot_report, "bot_report", "0", CVAR_CHEAT);
	cvarSystem->Register(&bot_testsolid, "bot_testsolid", "0", CVAR_CHEAT);
	cvarSystem->Register(&bot_testclusters, "bot_testclusters", "0", CVAR_CHEAT);
	cvarSystem->Register(&bot_developer, "bot_developer", "0", CVAR_CHEAT);
	cvarSystem->Register(&bot_interbreedchar, "bot_interbreedchar", "", 0);
	cvarSystem->Register(&bot_interbreedbots, "bot_interbreedbots", "10", 0);
	cvarSystem->Register(&bot_interbreedcycle, "bot_interbreedcycle", "20", 0);
	cvarSystem->Register(&bot_interbreedwrite, "bot_interbreedwrite", "", 0);
