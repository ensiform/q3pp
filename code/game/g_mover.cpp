...
gentity_t	*G_TestEntityPosition( gentity_t *ent ) {
	...
	if ( ent->client ) {
		trap->Trace( &tr, ent->client->ps.origin, ent->r.mins, ent->r.maxs, ent->client->ps.origin, ent->s.number, mask, false );
	} else {
		trap->Trace( &tr, ent->s.pos.trBase, ent->r.mins, ent->r.maxs, ent->s.pos.trBase, ent->s.number, mask, false );
	}
...
bool G_CheckProxMinePosition( gentity_t *check ) {
	...
	trap->Trace( &tr, start, NULL, NULL, end, check->s.number, MASK_SOLID, false );
...
void Reached_BinaryMover( gentity_t *ent ) {
	...
	if ( ent->moverState == MOVER_1TO2 ) {
		...
		if ( ent->teammaster == ent || !ent->teammaster ) {
			trap->AdjustAreaPortalState( ent, false );
		}
	} else {
...
void Use_BinaryMover( gentity_t *ent, gentity_t *other, gentity_t *activator ) {
	...
	if ( ent->moverState == MOVER_POS1 ) {
		...
		// open areaportal
		if ( ent->teammaster == ent || !ent->teammaster ) {
			trap->AdjustAreaPortalState( ent, true );
		}
		return;
	}
...
void SP_func_door (gentity_t *ent) {
	...
	trap->SetBrushModel( ent, ent->model );
...
void SP_func_train (gentity_t *self) {
	...
	trap->SetBrushModel( self, self->model );
	InitMover( self );
