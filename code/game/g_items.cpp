...
int Pickup_Powerup( gentity_t *ent, gentity_t *other ) {
    ...
    // cannot use OnSameTeam as it expects to g_entities, not clients
      ...
      continue;
		...
		trap->Trace( &tr, client->ps.origin, NULL, NULL, ent->s.pos.trBase, ENTITYNUM_NONE, CONTENTS_SOLID, false );
		if ( tr.fraction != 1.0 ) {
			continue;
		}
...
void FinishSpawningItem( gentity_t *ent ) {
	...
	if ( ent->spawnflags & 1 ) {
		...
		trap->Trace( &tr, ent->s.origin, ent->r.mins, ent->r.maxs, dest, ent->s.number, MASK_SOLID, false );
		if ( tr.startsolid ) {
...
int G_ItemDisabled( gitem_t *item ) {
	...
	return cvarSystem->VariableIntegerValue( name );
}
