...
int BotGetItemTeamGoal(char *goalname, bot_goal_t *goal) {
	...
	if (!strlen(goalname)) return false;
	i = -1;
	do {
		i = botlib->ai.BotGetLevelItemGoal(i, goalname, goal);
		if (i > 0) {
...
int BotGetMessageTeamGoal(bot_state_t *bs, char *goalname, bot_goal_t *goal) {
	...
	if (BotGetItemTeamGoal(goalname, goal)) return true;
...
float BotGetTime(bot_match_t *match) {
	...
	if (match->subtype & ST_TIME) {
		//get the time string
		botlib->ai.BotMatchVariable(match, TIME, timestring, MAX_MESSAGE_SIZE);
		//match it to find out if the time is in seconds or minutes
		if (botlib->ai.BotFindMatch(timestring, &timematch, MTCONTEXT_TIME)) {
			if (timematch.type == MSG_FOREVER) {
				...
				botlib->ai.BotMatchVariable(&timematch, TIME, timestring, MAX_MESSAGE_SIZE);
...
int NumPlayersOnSameTeam(bot_state_t *bs) {
	...
	num = 0;
	for (i = 0; i < maxclients && i < MAX_CLIENTS; i++) {
		trap->GetConfigstring(CS_PLAYERS+i, buf, MAX_INFO_STRING);
		if (strlen(buf)) {
...
int BotGetPatrolWaypoints(bot_state_t *bs, bot_match_t *match) {
	...
	patrolflags = 0;
	//
	botlib->ai.BotMatchVariable(match, KEYAREA, keyarea, MAX_MESSAGE_SIZE);
	//
	while(1) {
		if (!botlib->ai.BotFindMatch(keyarea, &keyareamatch, MTCONTEXT_PATROLKEYAREA)) {
			botlib->ea.EA_SayTeam(bs->client, "what do you say?");
			BotFreeWaypoints(newpatrolpoints);
			bs->patrolpoints = NULL;
			return false;
		}
		botlib->ai.BotMatchVariable(&keyareamatch, KEYAREA, keyarea, MAX_MESSAGE_SIZE);
		...
		if (keyareamatch.subtype & ST_BACK) {
			...
			botlib->ai.BotMatchVariable(&keyareamatch, MORE, keyarea, MAX_MESSAGE_SIZE);
		}
		else {
			break;
		}
	...
	if (!newpatrolpoints || !newpatrolpoints->next) {
		botlib->ea.EA_SayTeam(bs->client, "I need more key points to patrol\n");
		BotFreeWaypoints(newpatrolpoints);
		newpatrolpoints = NULL;
		return false;
	}
...
int BotAddressedToBot(bot_state_t *bs, bot_match_t *match) {
	...
	if (match->subtype & ST_ADDRESSED) {
		botlib->ai.BotMatchVariable(match, ADDRESSEE, addressedto, sizeof(addressedto));
		//the name of this bot
		ClientName(bs->client, botname, 128);
		//
		while(botlib->ai.BotFindMatch(addressedto, &addresseematch, MTCONTEXT_ADDRESSEE)) {
			if (addresseematch.type == MSG_EVERYONE) {
				return true;
			}
			else if (addresseematch.type == MSG_MULTIPLENAMES) {
				botlib->ai.BotMatchVariable(&addresseematch, TEAMMATE, name, sizeof(name));
				if (strlen(name)) {
					if (stristr(botname, name)) return true;
					if (stristr(bs->subteam, name)) return true;
				}
				botlib->ai.BotMatchVariable(&addresseematch, MORE, addressedto, MAX_MESSAGE_SIZE);
			}
			else {
				botlib->ai.BotMatchVariable(&addresseematch, TEAMMATE, name, MAX_MESSAGE_SIZE);
				if (strlen(name)) {
					if (stristr(botname, name)) return true;
					if (stristr(bs->subteam, name)) return true;
				}
		...
		if (!botlib->ai.BotFindMatch(match->string, &tellmatch, MTCONTEXT_REPLYCHAT) ||
				tellmatch.type != MSG_CHATTELL) {
			//make sure not everyone reacts to this message
			if (random() > (float ) 1.0 / (NumPlayersOnSameTeam(bs)-1)) return false;
		}
	}
	return true;
}
...
void BotMatch_HelpAccompany(bot_state_t *bs, bot_match_t *match) {
	...
	botlib->ai.BotMatchVariable(match, TEAMMATE, teammate, sizeof(teammate));
	//get the client to help
	if (botlib->ai.BotFindMatch(teammate, &teammatematch, MTCONTEXT_TEAMMATE) &&
	...
	if (client < 0) {
		...
		client = ClientFromName(netname);
		botlib->ai.BotEnterChat(bs->cs, client, CHAT_TELL);
		return;
	}
	...
	if (entinfo.valid) {
		...
		if (areanum) {// && botlib->aas.AAS_AreaReachability(areanum)) {
	...
	if (bs->teamgoal.entitynum < 0) {
		...
		if (match->subtype & ST_NEARITEM) {
			...
			botlib->ai.BotMatchVariable(match, ITEM, itemname, sizeof(itemname));
			//
			if (!BotGetMessageTeamGoal(bs, itemname, &bs->teamgoal)) {
				//BotAI_BotInitialChat(bs, "cannotfind", itemname, NULL);
				//botlib->ai.BotEnterChat(bs->cs, bs->client, CHAT_TEAM);
				return;
			}
		}
	}
	...
	if (bs->teamgoal.entitynum < 0) {
		...
		client = ClientFromName(netname);
		botlib->ai.BotEnterChat(bs->cs, client, CHAT_TEAM);
		return;
	}
	//the team mate
	bs->teammate = client;
...
void BotMatch_DefendKeyArea(bot_state_t *bs, bot_match_t *match) {
	...
	if (!BotAddressedToBot(bs, match)) return;
	//get the match variable
	botlib->ai.BotMatchVariable(match, KEYAREA, itemname, sizeof(itemname));
	//
	if (!BotGetMessageTeamGoal(bs, itemname, &bs->teamgoal)) {
		//BotAI_BotInitialChat(bs, "cannotfind", itemname, NULL);
		//botlib->ai.BotEnterChat(bs->cs, bs->client, CHAT_TEAM);
...
void BotMatch_TaskPreference(bot_state_t *bs, bot_match_t *match) {
	...
	botlib->ai.BotMatchVariable(match, NETNAME, teammatename, sizeof(teammatename));
	...
	botlib->ai.BotEnterChat(bs->cs, teammate, CHAT_TELL);
	BotVoiceChatOnly(bs, teammate, VOICECHAT_YES);
	botlib->ea.EA_Action(bs->client, ACTION_AFFIRMATIVE);
}
...
void BotMatch_JoinSubteam(bot_state_t *bs, bot_match_t *match) {
	...
	botlib->ai.BotMatchVariable(match, TEAMNAME, teammate, sizeof(teammate));
...
void BotMatch_CheckPoint(bot_state_t *bs, bot_match_t *match) {
	...
	if (!TeamPlayIsOn()) return;
	//
	botlib->ai.BotMatchVariable(match, POSITION, buf, MAX_MESSAGE_SIZE);
	...
	//
	botlib->ai.BotMatchVariable(match, NAME, buf, MAX_MESSAGE_SIZE);
...
void BotMatch_FormationSpace(bot_state_t *bs, bot_match_t *match) {
	...
	if (!BotAddressedToBot(bs, match)) return;
	//
	botlib->ai.BotMatchVariable(match, NUMBER, buf, MAX_MESSAGE_SIZE);
...
void BotMatch_Suicide(bot_state_t *bs, bot_match_t *match) {
	...
	if (!BotAddressedToBot(bs, match)) return;
	//
	botlib->ea.EA_Command(bs->client, "kill");
	//
	botlib->ai.BotMatchVariable(match, NETNAME, netname, sizeof(netname));
	client = ClientFromName(netname);
	//
...
void BotMatch_StartTeamLeaderShip(bot_state_t *bs, bot_match_t *match) {
	...
	if (match->subtype & ST_I) {
		...
		botlib->ai.BotMatchVariable(match, NETNAME, teammate, sizeof(teammate));
...
void BotMatch_WhoIsTeamLeader(bot_state_t *bs, bot_match_t *match) {
	...
	if (!Q_stricmp(netname, bs->teamleader)) {
		botlib->ea.EA_SayTeam(bs->client, "I'm the team leader\n");
	}
}
...
float BotNearestVisibleItem(bot_state_t *bs, char *itemname, bot_goal_t *goal) {
	...
	i = -1;
	do {
		i = botlib->ai.BotGetLevelItemGoal(i, itemname, &tmpgoal);
		botlib->ai.BotGoalName(tmpgoal.number, name, sizeof(name));
...
void BotMatch_WhereAreYou(bot_state_t *bs, bot_match_t *match) {
	...
	if (bestitem != -1) {
		if (gametype == GT_CTF
			...
			redtt = botlib->aas.AAS_AreaTravelTimeToGoalArea(bs->areanum, bs->origin, ctf_redflag.areanum, TFL_DEFAULT);
			bluett = botlib->aas.AAS_AreaTravelTimeToGoalArea(bs->areanum, bs->origin, ctf_blueflag.areanum, TFL_DEFAULT);
			...
			redtt = botlib->aas.AAS_AreaTravelTimeToGoalArea(bs->areanum, bs->origin, redobelisk.areanum, TFL_DEFAULT);
			bluett = botlib->aas.AAS_AreaTravelTimeToGoalArea(bs->areanum, bs->origin, blueobelisk.areanum, TFL_DEFAULT);
			if (redtt < (redtt + bluett) * 0.4) {
				BotAI_BotInitialChat(bs, "teamlocation", nearbyitems[bestitem], "red", NULL);
			}
			else if (bluett < (redtt + bluett) * 0.4) {
...
void BotMatch_Kill(bot_state_t *bs, bot_match_t *match) {
	...
	botlib->ai.BotMatchVariable(match, ENEMY, enemy, sizeof(enemy));
	//
...
void BotMatch_CTF(bot_state_t *bs, bot_match_t *match) {
	...
	if (gametype == GT_CTF) {
		botlib->ai.BotMatchVariable(match, FLAG, flag, sizeof(flag));
...
int BotMatchMessage(bot_state_t *bs, char *message) {
	...
	if (!botlib->ai.BotFindMatch(message, &match, MTCONTEXT_MISC
	...
	switch(match.type)
	{
		...
		{
			botlib->ea.EA_SayTeam(bs->client, "the part of my brain to create formations has been damaged");
			break;
		}
