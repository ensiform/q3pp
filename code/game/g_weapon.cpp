...
bool CheckGauntletAttack( gentity_t *ent ) {
	...
	trap->Trace (&tr, muzzle, NULL, NULL, end, ent->s.number, MASK_SHOT, false);
...
void Bullet_Fire (gentity_t *ent, float spread, int damage, int mod ) {
	...
	for (i = 0; i < 10; i++) {
		...
		trap->Trace (&tr, muzzle, NULL, NULL, end, passent, MASK_SHOT, false);
		if ( tr.surfaceFlags & SURF_NOIMPACT ) {
			return;
		}
...
bool ShotgunPellet( vec3_t start, vec3_t end, gentity_t *ent ) {
	...
	for (i = 0; i < 10; i++) {
		trap->Trace (&tr, tr_start, NULL, NULL, tr_end, passent, MASK_SHOT, false);
		traceEnt = &g_entities[ tr.entityNum ];
...
void weapon_railgun_fire (gentity_t *ent) {
	...
	passent = ent->s.number;
	do {
		trap->Trace (&trace, muzzle, NULL, NULL, end, passent, MASK_SHOT, false );
	...
	for ( i = 0 ; i < unlinked ; i++ ) {
		trap->LinkEntity( unlinkedEntities[i] );
	}
