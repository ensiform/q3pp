...
int BotValidChatPosition(bot_state_t *bs) {
	...
	if (bs->inventory[INVENTORY_QUAD] ||
		...
		bs->inventory[INVENTORY_FLIGHT]) return false;
	//must be on the ground
	//if (bs->cur_ps.groundEntityNum != ENTITYNUM_NONE) return false;
	//do not chat if in lava or slime
	VectorCopy(bs->origin, point);
	point[2] -= 24;
	if (trap->PointContents(point,bs->entitynum) & (CONTENTS_LAVA|CONTENTS_SLIME)) return false;
	//do not chat if under water
	VectorCopy(bs->origin, point);
	point[2] += 32;
	if (trap->PointContents(point,bs->entitynum) & MASK_WATER) return false;
	//must be standing on the world entity
	VectorCopy(bs->origin, start);
	VectorCopy(bs->origin, end);
	start[2] += 1;
	end[2] -= 10;
	botlib->aas.AAS_PresenceTypeBoundingBox(PRESENCE_CROUCH, mins, maxs);
	BotAI_Trace(&trace, start, mins, maxs, end, bs->client, MASK_SOLID);
	if (trace.ent != ENTITYNUM_WORLD) return false;
...
int BotChat_EnterGame(bot_state_t *bs) {
	...
	rnd = botlib->ai.Characteristic_BFloat(bs->character, CHARACTERISTIC_CHAT_ENTEREXITGAME, 0, 1);
	if (!bot_fastchat.integer) {
		if (random() > rnd) return false;
	}
...
int BotChat_StartLevel(bot_state_t *bs) {
	...
	if (bot_nochat.integer) return false;
	if (BotIsObserver(bs)) return false;
	if (bs->lastchat_time > FloatTime() - TIME_BETWEENCHATTING) return false;
	//don't chat in teamplay
	if (TeamPlayIsOn()) {
#ifdef MISSIONPACK
	    botlib->ea.EA_Command(bs->client, "vtaunt");
#endif
	    return false;
	}
	// don't chat in tournament mode
	if (gametype == GT_TOURNAMENT) return false;
	rnd = botlib->ai.Characteristic_BFloat(bs->character, CHARACTERISTIC_CHAT_STARTENDLEVEL, 0, 1);
	if (!bot_fastchat.integer) {
		if (random() > rnd) return false;
	}
	if (BotNumActivePlayers() <= 1) return false;
...
int BotChat_Death(bot_state_t *bs) {
	...
	if (bot_nochat.integer) return false;
	if (bs->lastchat_time > FloatTime() - TIME_BETWEENCHATTING) return false;
	rnd = botlib->ai.Characteristic_BFloat(bs->character, CHARACTERISTIC_CHAT_DEATH, 0, 1);
	// don't chat in tournament mode
	if (gametype == GT_TOURNAMENT) return false;
	...
	if (TeamPlayIsOn() && BotSameTeam(bs, bs->lastkilledby)) {
		if (bs->lastkilledby == bs->client) return false;
	...
	else
	{
		...
		if (bs->botdeathtype == MOD_WATER)
...
#ifdef MISSIONPACK
		else if (bs->botdeathtype == MOD_KAMIKAZE && botlib->ai.BotNumInitialChats(bs->cs, "death_kamikaze"))
			...
			else if (random() < botlib->ai.Characteristic_BFloat(bs->character, CHARACTERISTIC_CHAT_INSULT, 0, 1)) {
...
int BotChat_Kill(bot_state_t *bs) {
	...
	if (bot_nochat.integer) return false;
	if (bs->lastchat_time > FloatTime() - TIME_BETWEENCHATTING) return false;
	rnd = botlib->ai.Characteristic_BFloat(bs->character, CHARACTERISTIC_CHAT_KILL, 0, 1);
	// don't chat in tournament mode
	if (gametype == GT_TOURNAMENT) return false;
	//if fast chat is off
	if (!bot_fastchat.integer) {
	...
	if (bs->lastkilledplayer == bs->client) return false;
	if (BotNumActivePlayers() <= 1) return false;
	if (!BotValidChatPosition(bs)) return false;
	//
	if (BotVisibleEnemies(bs)) return false;
	//
...
#ifdef MISSIONPACK
		else if (bs->botdeathtype == MOD_KAMIKAZE && botlib->ai.BotNumInitialChats(bs->cs, "kill_kamikaze"))
...
int BotChat_EnemySuicide(bot_state_t *bs) {
	...
	if (BotNumActivePlayers() <= 1) return false;
	//
	rnd = botlib->ai.Characteristic_BFloat(bs->character, CHARACTERISTIC_CHAT_ENEMYSUICIDE, 0, 1);
	//don't chat in teamplay
	if (TeamPlayIsOn()) return false;
	// don't chat in tournament mode
	if (gametype == GT_TOURNAMENT) return false;
...
int BotChat_HitTalking(bot_state_t *bs) {
	...
	//
	if (lasthurt_client < 0 || lasthurt_client >= MAX_CLIENTS) return false;
	//
	rnd = botlib->ai.Characteristic_BFloat(bs->character, CHARACTERISTIC_CHAT_HITTALKING, 0, 1);
	//don't chat in teamplay
	if (TeamPlayIsOn()) return false;
	// don't chat in tournament mode
	if (gametype == GT_TOURNAMENT) return false;
	//if fast chat is off
	if (!bot_fastchat.integer) {
		if (random() > rnd * 0.5) return false;
	}
	if (!BotValidChatPosition(bs)) return false;
	//
...
int BotChat_HitNoDeath(bot_state_t *bs) {
	...
	if (bs->lastchat_time > FloatTime() - TIME_BETWEENCHATTING) return false;
	if (BotNumActivePlayers() <= 1) return false;
	rnd = botlib->ai.Characteristic_BFloat(bs->character, CHARACTERISTIC_CHAT_HITNODEATH, 0, 1);
	//don't chat in teamplay
	if (TeamPlayIsOn()) return false;
	// don't chat in tournament mode
	if (gametype == GT_TOURNAMENT) return false;
...
int BotChat_HitNoKill(bot_state_t *bs) {
	...
	if (bs->lastchat_time > FloatTime() - TIME_BETWEENCHATTING) return false;
	if (BotNumActivePlayers() <= 1) return false;
	rnd = botlib->ai.Characteristic_BFloat(bs->character, CHARACTERISTIC_CHAT_HITNOKILL, 0, 1);
	//don't chat in teamplay
	if (TeamPlayIsOn()) return false;
	// don't chat in tournament mode
	if (gametype == GT_TOURNAMENT) return false;
...
int BotChat_Random(bot_state_t *bs) {
	...
	if (bs->ltgtype == LTG_TEAMHELP ||
		...
		bs->ltgtype == LTG_RUSHBASE) return false;
	//
	rnd = botlib->ai.Characteristic_BFloat(bs->character, CHARACTERISTIC_CHAT_RANDOM, 0, 1);
	...
	//
	if (random() < botlib->ai.Characteristic_BFloat(bs->character, CHARACTERISTIC_CHAT_MISC, 0, 1)) {
...
float BotChatTime(bot_state_t *bs) {
	...
	//cpm = botlib->ai.Characteristic_BInteger(bs->character, CHARACTERISTIC_CHAT_CPM, 1, 4000);

	return 2.0;	//(float) botlib->ai.BotChatLength(bs->cs) * 30 / cpm;
}
...
void BotChatTest(bot_state_t *bs) {
	...
	num = botlib->ai.BotNumInitialChats(bs->cs, "game_enter");
	for (i = 0; i < num; i++)
	{
		BotAI_BotInitialChat(bs, "game_enter",
					EasyClientName(bs->client, name, 32),	// 0
					...
					BotMapTitle(),							// 4
					NULL);
		botlib->ai.BotEnterChat(bs->cs, 0, CHAT_ALL);
	}
	num = botlib->ai.BotNumInitialChats(bs->cs, "game_exit");
	for (i = 0; i < num; i++)
	{
		BotAI_BotInitialChat(bs, "game_exit",
					EasyClientName(bs->client, name, 32),	// 0
	...
	num = botlib->ai.BotNumInitialChats(bs->cs, "level_start");
	for (i = 0; i < num; i++)
	{
		BotAI_BotInitialChat(bs, "level_start",
					EasyClientName(bs->client, name, 32),	// 0
	...
	num = botlib->ai.BotNumInitialChats(bs->cs, "level_end_victory");
	for (i = 0; i < num; i++)
	{
		BotAI_BotInitialChat(bs, "level_end_victory",
				EasyClientName(bs->client, name, 32),	// 0
	...
	num = botlib->ai.BotNumInitialChats(bs->cs, "level_end_lose");
	for (i = 0; i < num; i++)
	{
		BotAI_BotInitialChat(bs, "level_end_lose",
				EasyClientName(bs->client, name, 32),	// 0
	...
	num = botlib->ai.BotNumInitialChats(bs->cs, "level_end");
	for (i = 0; i < num; i++)
	{
		BotAI_BotInitialChat(bs, "level_end",
				EasyClientName(bs->client, name, 32),	// 0
	...
	num = botlib->ai.BotNumInitialChats(bs->cs, "death_drown");
	for (i = 0; i < num; i++)
	{
		//
	...
	num = botlib->ai.BotNumInitialChats(bs->cs, "death_slime");
	for (i = 0; i < num; i++)
	{
		BotAI_BotInitialChat(bs, "death_slime", name, NULL);
		botlib->ai.BotEnterChat(bs->cs, 0, CHAT_ALL);
	}
	num = botlib->ai.BotNumInitialChats(bs->cs, "death_lava");
	for (i = 0; i < num; i++)
	{
		BotAI_BotInitialChat(bs, "death_lava", name, NULL);
		botlib->ai.BotEnterChat(bs->cs, 0, CHAT_ALL);
	}
	num = botlib->ai.BotNumInitialChats(bs->cs, "death_cratered");
	for (i = 0; i < num; i++)
	{
		BotAI_BotInitialChat(bs, "death_cratered", name, NULL);
		botlib->ai.BotEnterChat(bs->cs, 0, CHAT_ALL);
	}
	num = botlib->ai.BotNumInitialChats(bs->cs, "death_suicide");
	for (i = 0; i < num; i++)
	{
		BotAI_BotInitialChat(bs, "death_suicide", name, NULL);
		botlib->ai.BotEnterChat(bs->cs, 0, CHAT_ALL);
	}
	num = botlib->ai.BotNumInitialChats(bs->cs, "death_telefrag");
	for (i = 0; i < num; i++)
	{
		BotAI_BotInitialChat(bs, "death_telefrag", name, NULL);
		botlib->ai.BotEnterChat(bs->cs, 0, CHAT_ALL);
	}
	num = botlib->ai.BotNumInitialChats(bs->cs, "death_gauntlet");
	for (i = 0; i < num; i++)
	{
		BotAI_BotInitialChat(bs, "death_gauntlet",
				name,												// 0
	...
	num = botlib->ai.BotNumInitialChats(bs->cs, "death_rail");
	for (i = 0; i < num; i++)
	{
		BotAI_BotInitialChat(bs, "death_rail",
				name,												// 0
	...
	num = botlib->ai.BotNumInitialChats(bs->cs, "death_bfg");
	for (i = 0; i < num; i++)
	{
		BotAI_BotInitialChat(bs, "death_bfg",
				name,												// 0
	...
	num = botlib->ai.BotNumInitialChats(bs->cs, "death_insult");
	for (i = 0; i < num; i++)
	{
		BotAI_BotInitialChat(bs, "death_insult",
					name,												// 0
	...
	num = botlib->ai.BotNumInitialChats(bs->cs, "death_praise");
	for (i = 0; i < num; i++)
	{
		BotAI_BotInitialChat(bs, "death_praise",
					name,												// 0
	...
	EasyClientName(bs->lastkilledplayer, name, 32);
	//
	num = botlib->ai.BotNumInitialChats(bs->cs, "kill_gauntlet");
	for (i = 0; i < num; i++)
	{
		//
		BotAI_BotInitialChat(bs, "kill_gauntlet", name, NULL);
	...
	num = botlib->ai.BotNumInitialChats(bs->cs, "kill_rail");
	for (i = 0; i < num; i++)
	{
		BotAI_BotInitialChat(bs, "kill_rail", name, NULL);
		botlib->ai.BotEnterChat(bs->cs, 0, CHAT_ALL);
	}
	num = botlib->ai.BotNumInitialChats(bs->cs, "kill_telefrag");
	for (i = 0; i < num; i++)
	{
		BotAI_BotInitialChat(bs, "kill_telefrag", name, NULL);
		botlib->ai.BotEnterChat(bs->cs, 0, CHAT_ALL);
	}
	num = botlib->ai.BotNumInitialChats(bs->cs, "kill_insult");
	for (i = 0; i < num; i++)
	{
		BotAI_BotInitialChat(bs, "kill_insult", name, NULL);
		botlib->ai.BotEnterChat(bs->cs, 0, CHAT_ALL);
	}
	num = botlib->ai.BotNumInitialChats(bs->cs, "kill_praise");
	for (i = 0; i < num; i++)
	{
		BotAI_BotInitialChat(bs, "kill_praise", name, NULL);
		botlib->ai.BotEnterChat(bs->cs, 0, CHAT_ALL);
	}
	num = botlib->ai.BotNumInitialChats(bs->cs, "enemy_suicide");
	for (i = 0; i < num; i++)
	{
		BotAI_BotInitialChat(bs, "enemy_suicide", name, NULL);
		botlib->ai.BotEnterChat(bs->cs, 0, CHAT_ALL);
	...
	num = botlib->ai.BotNumInitialChats(bs->cs, "hit_talking");
	for (i = 0; i < num; i++)
	{
		BotAI_BotInitialChat(bs, "hit_talking", name, weap, NULL);
		botlib->ai.BotEnterChat(bs->cs, 0, CHAT_ALL);
	}
	num = botlib->ai.BotNumInitialChats(bs->cs, "hit_nodeath");
	for (i = 0; i < num; i++)
	{
		BotAI_BotInitialChat(bs, "hit_nodeath", name, weap, NULL);
		botlib->ai.BotEnterChat(bs->cs, 0, CHAT_ALL);
	}
	num = botlib->ai.BotNumInitialChats(bs->cs, "hit_nokill");
	for (i = 0; i < num; i++)
	{
		BotAI_BotInitialChat(bs, "hit_nokill", name, weap, NULL);
		botlib->ai.BotEnterChat(bs->cs, 0, CHAT_ALL);
	...
	//
	num = botlib->ai.BotNumInitialChats(bs->cs, "random_misc");
	for (i = 0; i < num; i++)
	{
		//
		BotAI_BotInitialChat(bs, "random_misc",
	...
	num = botlib->ai.BotNumInitialChats(bs->cs, "random_insult");
	for (i = 0; i < num; i++)
	{
		BotAI_BotInitialChat(bs, "random_insult",
					BotRandomOpponentName(bs),	// 0
