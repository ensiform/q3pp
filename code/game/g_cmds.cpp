...
bool	CheatsOk( gentity_t *ent ) {
	...
	if ( ent->health <= 0 ) {
		trap->SendServerCommand( ent-g_entities, "print \"You must be alive to use this command.\n\"");
		return false;
	}
	return true;
}
...
void BroadcastTeamChange( gclient_t *client, int oldTeam )
{
	if ( client->sess.sessionTeam == TEAM_RED ) {
		trap->SendServerCommand( -1, va("cp \"%s" S_COLOR_WHITE " joined the red team.\n\"",
			client->pers.netname) );
	} else if ( client->sess.sessionTeam == TEAM_BLUE ) {
		trap->SendServerCommand( -1, va("cp \"%s" S_COLOR_WHITE " joined the blue team.\n\"",
		client->pers.netname));
	} else if ( client->sess.sessionTeam == TEAM_SPECTATOR && oldTeam != TEAM_SPECTATOR ) {
		trap->SendServerCommand( -1, va("cp \"%s" S_COLOR_WHITE " joined the spectators.\n\"",
		client->pers.netname));
	} else if ( client->sess.sessionTeam == TEAM_FREE ) {
		trap->SendServerCommand( -1, va("cp \"%s" S_COLOR_WHITE " joined the battle.\n\"",
		client->pers.netname));
	}
}
...
void Cmd_Team_f( gentity_t *ent ) {
	...
	if ( trap->Cmd_Argc() != 2 ) {
		...
		case TEAM_BLUE:
			...
			break;
		case TEAM_RED:
			trap->SendServerCommand( ent-g_entities, "print \"Red team\n\"" );
			break;
		case TEAM_FREE:
			trap->SendServerCommand( ent-g_entities, "print \"Free team\n\"" );
			break;
		case TEAM_SPECTATOR:
...
static void G_VoiceTo( gentity_t *ent, gentity_t *other, int mode, const char *id, bool voiceonly ) {
	...
	trap->SendServerCommand( other-g_entities, va("%s %d %d %d %s", cmd, voiceonly, ent->s.number, color, id));
}
...
static void Cmd_VoiceTell_f( gentity_t *ent, bool voiceonly ) {
	...
	if ( trap->Cmd_Argc () < 3 ) {
		trap->SendServerCommand( ent-g_entities, va( "print \"Usage: %s <player id> <voice id>\n\"", voiceonly ? "votell" : "vtell" ) );
		return;
	}
...
void Cmd_CallVote_f( gentity_t *ent ) {
	...
	if ( !g_allowVote.integer ) {
		trap->SendServerCommand( ent-g_entities, "print \"Voting not allowed here.\n\"" );
		return;
	}

	if ( level.voteTime ) {
		trap->SendServerCommand( ent-g_entities, "print \"A vote is already in progress.\n\"" );
		return;
	}
	if ( ent->client->pers.voteCount >= MAX_VOTE_COUNT ) {
		trap->SendServerCommand( ent-g_entities, "print \"You have called the maximum number of votes.\n\"" );
		return;
	}
	if ( ent->client->sess.sessionTeam == TEAM_SPECTATOR ) {
		trap->SendServerCommand( ent-g_entities, "print \"Not allowed to call a vote as spectator.\n\"" );
		return;
	}
	...
	if ( !Q_stricmp( arg1, "map_restart" ) ) {
		...
		trap->SendServerCommand( ent-g_entities, "print \"Invalid vote string.\n\"" );
		trap->SendServerCommand( ent-g_entities, "print \"Vote commands are: map_restart, nextmap, map <mapname>, g_gametype <n>, kick <player>, clientkick <clientnum>, g_doWarmup, timelimit <time>, fraglimit <frags>.\n\"" );
		return;
	}
	...
	if ( !Q_stricmp( arg1, "g_gametype" ) ) {
		...
		cvarSystem->VariableStringBuffer( "nextmap", s, sizeof(s) );
		if (*s) {
	...
	trap->SendServerCommand( -1, va("print \"%s called a vote.\n\"", ent->client->pers.netname ) );
...
void Cmd_Vote_f( gentity_t *ent ) {
	...
	if ( !level.voteTime ) {
		trap->SendServerCommand( ent-g_entities, "print \"No vote in progress.\n\"" );
		return;
	}
	if ( ent->client->ps.eFlags & EF_VOTED ) {
		trap->SendServerCommand( ent-g_entities, "print \"Vote already cast.\n\"" );
		return;
	}
	if ( ent->client->sess.sessionTeam == TEAM_SPECTATOR ) {
		trap->SendServerCommand( ent-g_entities, "print \"Not allowed to vote as spectator.\n\"" );
		return;
	}

	trap->SendServerCommand( ent-g_entities, "print \"Vote cast.\n\"" );
...
void Cmd_CallTeamVote_f( gentity_t *ent ) {
	...
	if ( level.teamVoteTime[cs_offset] ) {
		trap->SendServerCommand( ent-g_entities, "print \"A team vote is already in progress.\n\"" );
		return;
	}
	if ( ent->client->pers.teamVoteCount >= MAX_VOTE_COUNT ) {
		trap->SendServerCommand( ent-g_entities, "print \"You have called the maximum number of team votes.\n\"" );
		return;
	}
	if ( ent->client->sess.sessionTeam == TEAM_SPECTATOR ) {
		trap->SendServerCommand( ent-g_entities, "print \"Not allowed to call a vote as spectator.\n\"" );
	...
	trap->Cmd_Argv( 1, arg1, sizeof( arg1 ) );
	arg2[0] = '\0';
	for ( i = 2; i < trap->Cmd_Argc(); i++ ) {
		if (i > 2)
			strcat(arg2, " ");
		trap->Cmd_Argv( i, &arg2[strlen(arg2)], sizeof( arg2 ) - strlen(arg2) );
	}
...
void Cmd_TeamVote_f( gentity_t *ent ) {
	...
	if ( !level.teamVoteTime[cs_offset] ) {
		trap->SendServerCommand( ent-g_entities, "print \"No team vote in progress.\n\"" );
		return;
	}
	if ( ent->client->ps.eFlags & EF_TEAMVOTED ) {
		trap->SendServerCommand( ent-g_entities, "print \"Team vote already cast.\n\"" );
		return;
	}
	if ( ent->client->sess.sessionTeam == TEAM_SPECTATOR ) {
		trap->SendServerCommand( ent-g_entities, "print \"Not allowed to vote as spectator.\n\"" );
	...
	trap->SendServerCommand( ent-g_entities, "print \"Team vote cast.\n\"" );
...
void Cmd_SetViewpos_f( gentity_t *ent ) {
	...
	if ( trap->Cmd_Argc() != 5 ) {
		trap->SendServerCommand( ent-g_entities, "print \"usage: setviewpos x y z yaw\n\"");
		return;
	}

	VectorClear( angles );
	for ( i = 0 ; i < 3 ; i++ ) {
		trap->Cmd_Argv( i + 1, buffer, sizeof( buffer ) );
		origin[i] = atof( buffer );
	}

	trap->Cmd_Argv( 4, buffer, sizeof( buffer ) );
...
/*
	...
	max = botlib->aas.AAS_PointReachabilityAreaIndex( NULL );
	...
	//trap->SendServerCommand( ent-g_entities, va("print \"visited %d of %d areas\n\"", n, max));
	trap->SendServerCommand( ent-g_entities, va("print \"%d%% level coverage\n\"", n * 100 / max));
*/
}
